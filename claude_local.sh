...
# Assumes mcp-tools-sql is editable-installed (pip install -e .)
...
if ! "$PROJECT_VENV/bin/python" -c "from importlib.metadata import distribution as D; u=D('mcp-tools-sql').read_text('direct_url.json') or ''; exit(0 if 'dir_info' in u and 'editable' in u else 1)" 2>/dev/null; then
    echo "WARNING: mcp-tools-sql does not appear to be editable-installed from $PWD"
    echo "  For development, run: tools/reinstall_local.sh"
