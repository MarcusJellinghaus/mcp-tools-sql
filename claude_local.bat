...
REM Assumes mcp-tools-sql is editable-installed (pip install -e .)
...
"%CD%\.venv\Scripts\python.exe" -c "from importlib.metadata import distribution as D; u=D('mcp-tools-sql').read_text('direct_url.json') or ''; exit(0 if 'dir_info' in u and 'editable' in u else 1)" 2>nul
if !errorlevel! NEQ 0 (
    echo WARNING: mcp-tools-sql does not appear to be editable-installed from %CD%
    echo   For development, run: tools\reinstall_local.bat
