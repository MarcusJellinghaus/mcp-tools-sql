...
- **Data profiling**: `summarize_columns(schema?, table?, sql?, columns?, where?, params?, n?)` — per-column row/null/distinct counts, category-appropriate statistics, and duplication-driven value lists (top values with frequencies, or a sample when every value is unique). The source is either a persisted table (`schema`+`table`) or an arbitrary read-only SELECT (`sql`), never both; with `sql` the `where` predicate is applied outside the source, so it can filter computed and aggregated columns. The value list subsumes `read_distinct_values`. Compact overview, not full data.
