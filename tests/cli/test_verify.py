...
# ---------------------------------------------------------------------------
# verify_updates: identifier whitelist + required flag visibility (Step 6)
# ---------------------------------------------------------------------------


def test_verify_updates_rejects_invalid_table_identifier(sqlite_db: Path) -> None:
...
def test_verify_updates_rejects_invalid_schema_identifier(sqlite_db: Path) -> None:
    ...
    backend = _open_sqlite_backend(sqlite_db)
    try:
        ok_result = verify_cmd.verify_updates(ok_updates, "sqlite", backend)
    finally:
        backend.close()
    assert ok_result["good.table"]["ok"] is True


def test_verify_updates_rejects_invalid_key_field_identifier(sqlite_db: Path) -> None:
...
def test_verify_updates_rejects_invalid_field_identifier(sqlite_db: Path) -> None:
