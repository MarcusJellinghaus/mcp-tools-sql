...
core(...):
    ...
    if err: return err
    try: return _run(backend, rec, built, predicate, params, columns, n, dialect)
    except _INVALID_SQL_EXC as exc:
        return f"Invalid SQL. {type(exc).__name__}: {exc}" + (LOB_HINT if built.types_probed else "")
```
...
>
> Write the end-to-end tests listed under TESTS first. Test 13 is a regression guard: the
> `schema`/`table` path must produce byte-identical output.
>
