...
stderr, making the file the only durable record. Log files are never rotated
or deleted, and the server will fail to start if `~/.mcp-tools-sql/logs/`
cannot be created.
...
> Leave the stale `--project-dir` at ~line 718 alone — adjacent, but a
...
>
> Implement Step 6 only — documentation, no code:
...
>    mcp-workspace / mcp-tools-py prior art. Leave the stale `--project-dir`
>    line alone.
